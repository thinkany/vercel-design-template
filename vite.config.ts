...
function templateZipPlugin() {
  return {
    ...
    async generateBundle(this: any) {
      ...
      // git-tracked files = the distributable template source, MINUS the
      // app-internal export-to-Figma tooling (app-owned, runs from the Electron
      // app). This is the single choke point: the upgrade overlay can only write
