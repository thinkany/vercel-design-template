...
//
//   node media-convert.cjs <in> <out.avif|out.webp|out.jpg> [maxWidth] [quality]   → prints JSON
//
...
// JPEG is for the alt-text call (main.cjs describeImageAlt): the API reads no AVIF
// or SVG, so any library image is rendered small, on white, for the model to see.
...
async function main() {
  ...
  const encoded = jpeg ? resized.flatten({ background: "#fff" }).jpeg({ quality: Number(quality) })
    : avif ? resized.avif({ quality: Number(quality), effort: 4 })
    : resized.webp({ quality: Number(quality), effort: 4 });
