...
//
//   node media-convert.cjs <in> <out.webp> [maxWidth] [quality]   → prints JSON
//
// Auto-orients from EXIF, strips metadata (sharp's default), never upscales.
const path = require("node:path");

async function main() {
  const [inPath, outPath, maxW = "2400", quality = "82"] = process.argv.slice(2);
  if (!inPath || !outPath) throw new Error("usage: media-convert <in> <out.webp> [maxWidth] [quality]");
