  ...
  site: {
    ...
    settings: {
      ...
      wp: {
        ...
        pluginHint: "Save the plugin, upload it on the WordPress site (Plugins → Add New → Upload), activate it, then open Settings → thinkany Export for the token. It only reads, so it's safe on a live site; deactivate it once you're done.",
        savePlugin: "Save the plugin…",
        pluginSaved: (p) => `Saved to ${p}.`,
        fetchStep: "2. The content",
        url: "Site address",
        urlPlaceholder: "https://client-site.com",
        token: "Token",
        tokenPlaceholder: "From Settings → thinkany Export",
        ...
        mapStep: "3. The mapping",
        mapHint: "Old blocks and fields need a destination among this site's blocks. Propose one from the site's blocks, then confirm or edit the file before importing.",
        propose: "Propose a mapping",
        proposeRequest: "/migrate-wordpress map",
        proposeEcho: "Propose a WordPress import mapping",
        mappingReady: "A mapping is in place. Open the file to check or change it before importing.",
        mappingEmpty: "The mapping file exists but has no destinations yet. Propose one, or fill it in by hand.",
        mappingMissing: "No mapping yet.",
        reveal: "Show the mapping file",
        restart: "Start the mapping over",
        restartConfirm: "Set the current mapping aside (kept beside it, dated) and write a fresh one from the site's blocks?",
        restarted: "A fresh mapping is in place. Propose a mapping fills it in.",
        importStep: "4. The import",
        importHint: "Writes the pages, posts, entries and images into this site, and a redirect for every old address that changed. Re-run it after editing the mapping; it overwrites what it wrote and nothing else.",
        run: "Run the import",
        running: "Importing…",
        needSite: "Build the site from the approved design first. The import writes into the site's blocks.",
        needMapping: "Add a mapping first.",
        needFilled: "The mapping has no destinations yet.",
        done: (r) => `${r.pages === 1 ? "1 page" : `${r.pages} pages`}, ${r.posts === 1 ? "1 post" : `${r.posts} posts`}, ${r.redirects === 1 ? "1 redirect" : `${r.redirects} redirects`}, ${r.media.downloaded} images${r.media.failed.length ? ` (${r.media.failed.length} failed)` : ""}${r.unmappedBlocks ? `, ${r.unmappedBlocks} block type${r.unmappedBlocks === 1 ? "" : "s"} without a destination` : ""}.`,
