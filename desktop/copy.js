...
window.COPY = {
  ...
  site: {
    ...
    tabs: { pages: "Pages", posts: "Posts", types: "Types", forms: "Forms", media: "Media", blocks: "Blocks", nav: "Navigation", competitors: "Competitors", settings: "Settings" },
    // ── Competitors tab (docs/competitor-review-spec.md) ──
    competitors: {
      listLabel: "Competitor sites",
      empty: "Add the sites this one competes with, up to six.",
      seeded: "Started from the sites named in the brief. Add or remove any before reviewing.",
      addPlaceholder: "example.com",
      add: "Add",
      max: "Six is the most a review reads.",
      remove: "Remove",
      removeAria: (host) => `Remove ${host}`,
      review: "Review the field",
      reviewAgain: "Review again",
      running: "A review is running.",
      busy: "Wait for the current turn to finish.",
      notYet: "Not reviewed yet.",
      lastRun: (when, read, failed) => `Last reviewed ${when}: ${read} site${read === 1 ? "" : "s"} read${failed ? `, ${failed} could not be read` : ""}.`,
      notLicensed: "The competitor review is part of the Design license. Add your key under Keys & Licenses.",
      needKey: "Add your Claude API key to run a review.",
      intro: "Claude reads each competitor’s home page and its main pages, compares what they show against what this site has, and comes back with the gaps to close and the openings nobody in the field has taken. Each recommendation carries its evidence and, where the builder can make the change, an Apply button.",
      progressReading: (done, total, host, failed) => `Reading the field: ${done} of ${total}${host ? ` (${host}${failed ? ", could not be read" : ""})` : ""}…`,
      progressReviewing: (n) => `Comparing ${n} site${n === 1 ? "" : "s"} against yours…`,
      progressDone: "Review finished.",
      progressError: (m) => `The review stopped: ${m}`,
      readMore: "Read the full report",
      readLess: "Show less",
      colSite: "Site",
      thisSite: "This site",
      you: "you",
      unreadable: "This site could not be read.",
      columns: { pricing: "Pricing", faq: "FAQ", testimonials: "Testimonials", logos: "Logos", stats: "Stats", form: "Form", blog: "Blog", cta: "Primary CTA" },
      edge: "Edge",
      opportunity: "Opportunity",
      groupEdge: "Close the gap",
      groupOpportunity: "Take the opening",
      groupHeld: "Held",
      groupDone: "Done",
      groupDismissed: "Dismissed",
      noRecs: "No recommendations from this review.",
      pickOne: "Pick a recommendation to see its evidence and the section it lands on.",
      kinds: { code: "Actionable", create: "New page", asset: "Needs an asset", decision: "Your call" },
      evidence: "Evidence",
      where: (page, block) => (block ? `On the ${page} page, the ${block} section.` : `On the ${page} page.`),
      createPlan: (title, blocks) => `A new page, "${title}", built from the ${blocks} blocks the site already has.`,
      apply: "Apply",
      ...
      reopen: "Reopen",
      restore: "Restore",
      doneNote: "Applied. Check the Site tab.",
      assetNote: "This needs material only the client can supply.",
      decisionNote: "This is a call for you or the client to make.",
      applyingEcho: (title) => `Apply: ${title}`,
    ...
    // the built-in Types block (its own editor: listing or picker)
    entriesBlock: {
      heading: "Heading",
      mode: "Show",
      listing: "A listing",
      picker: "Picked entries",
      types: "From these types",
      typesHint: "With more than one type, entries interleave newest first. Drafts show only while previewing.",
      noTypes: "No content types yet. Make one in the Types tab.",
      limit: "How many to show",
      limitHint: "0 shows every entry.",
      paginate: "Split into pages",
      perPage: "Per page",
      filter: "Let visitors filter by Tag/Type",
      filterHint: "Pills above the listing: each type, and each tag the shown entries carry.",
      filterNeedsTwo: "Pick two or more types to offer a filter.",
      picks: "Entries",
      picksHint: "Shown in this order. Add as many as you like. A draft shows only while previewing.",
      addPick: "+ Add an entry",
      pickType: "Choose a type…",
      pickEntry: "Choose an entry…",
      pickMissing: (id) => `${id} (missing)`,
      linkStyle: "Link",
      linkCard: "The card is the link",
      linkMore: "A “Read more” line",
      linkLabel: "Link words",
    ...
    help: {
      ...
      competitors: {
        title: "Competitors",
        intro: "Have Claude read the sites this one competes with and tell you where this site is behind, and where it could lead.",
        sections: [
          { h: "The list", items: ["Add up to six competitor sites. The list starts from the sites named in the brief, if any.", "<b>Review the field</b> reads each site’s home page and its main pages (not contact, login or legal pages), reads this site from its own content, and asks Claude to compare them. It takes a minute or two."] },
          { h: "The table", items: ["One row per site, this one first. A tick or a count shows what each site has on its pages: pricing, a FAQ, testimonials, client logos, numbers, a form, a blog, and the first call to action.", "A <b>?</b> row is a site that could not be read (some block automated reads)."] },
          { h: "Recommendations", items: ["<b>Close the gap</b>: things the field does that this site does not, or does weakly. <b>Take the opening</b>: things nobody in the field does that this site is placed to own.", "Each one carries its evidence, counted from the read, and shows the section it lands on, live.", "<b>Apply</b> makes the change in the site’s blocks or content through a short builder turn in the chat. <b>Create the page</b> adds a new page and fills it from the blocks the site already has. <b>Hold</b> keeps one for later; <b>Dismiss</b> archives it.", "Claude never copies a competitor’s copy, layout or imagery: a recommendation names the gap, not their solution."] },
        ],
      },
      settings: {
        title: "Settings",
        ...
        sections: [
          ...
          { h: "Blog", items: ["<b>Create with AI</b>, above the Filter section: give a title and a brief, and Claude writes the post in the site's copy voice (the Copy Voice drawer in the rail), with its summary, tags and search fields. It lands as a draft, open for your review; nothing publishes until you do.", "<b>Posts block</b>: add the Posts block to any page from the block picker to show the newest posts there. Set how many it shows, and whether visitors can filter them by tag (as pills or a select list), here; every Posts block on the site follows these settings.", "<b>Posts Directory</b> is the address posts are listed under and served from (<i>/blog</i>, <i>/blog/my-post</i>). Change it to <i>news</i>, <i>journal</i>, whatever fits, and choose a <b>Parent page</b> to put the blog under it (<i>/resources/blog</i>). Leave the directory empty to put posts right under the parent page (<i>/resources/my-post</i>), which is then the list. Tick <b>Group posts by their first tag</b> to put each post under its first tag (<i>/blog/news/my-post</i>), with a list per tag. Menu links to posts follow any of these changes, and no page may take a post's address."] },
    ...
    aiPostHeading: "Create with AI",
    aiPostDesc: "Give Claude a title and a brief. It writes the post in the site's copy voice, with a summary, tags and search fields, and opens it here as a draft for you to review.",
    aiPostTitle: "Title",
    aiPostTitlePlaceholder: "The post's title",
    aiPostBrief: "Brief",
    aiPostBriefPlaceholder: "What the post should say: the point, who it's for, facts to include, anything to leave out.",
    aiPostBriefHint: "The more you give, the less it has to guess. It never invents facts or figures; where the brief has none, it writes around them.",
    aiPostWrite: "Write the post",
    aiPostWriteTitle: "Write this post with Claude, in the site's copy voice",
    aiPostWorking: "Writing the post",
    aiPostNeedTitle: "Give the post a title first.",
    ...
    entryTagsHeading: "Tags",
    postTags: "Tags",
    postTagsHint: "Pick a tag other posts use, or type a new one and press Enter. Drag the chips to reorder them, or star one to make it Primary. Tags save with the post.",
    postTagsHintFirst: "Pick a tag other posts use, or type a new one and press Enter. Drag the chips to reorder them, or star one to make it Primary: the Primary tag is part of the post's address (Settings → Blog), so changing it moves the post; menu links follow.",
    ...
    editor: {
      ...
      ai: "Ask Claude to edit this text",
      aiAsk: "Ask Claude",
      aiPlaceholder: "What should change? e.g. tighten this to half the length, add a closing line about the guarantee, make the tone warmer",
      aiApply: "Rewrite",
      aiWorking: "Rewriting",
      aiDone: "Rewritten. Undo brings the previous text back.",
      aiFail: "Couldn't rewrite the text.",
    },
    savePost: "Save post",
    ...
    typeDataOnly: "Data only: entries get no page of their own",
    typeDataOnlyHint: "For content other things use, like a Reference field on another type or a block that lists entries. No address, no index, no template.",
    dataOnlyTag: "data only",
    ...
    addField: "+ Add field",
    kinds: { text: "Text", textarea: "Long text", richtext: "Rich text", number: "Number", boolean: "Yes / no", date: "Date", image: "Image", select: "Choice", list: "List of text", link: "Link", reference: "Reference to another type", tags: "Tags" },
    fieldTagsHint: "Tags stay within this type: the values entered here are offered only to its other entries, never to another type, even one with a field of the same name. The list can then be filtered by them.",
    entryTagsHint: (s) => `Pick a tag other ${s} use, or type a new one and press Enter. Tags save with the entry.`,
    entryNoTags: "No tags yet.",
    ...
    settings: {
      blogHeading: "Blog",
      postsParent: "Parent page",
      postsParentNone: "None (top level)",
      ...
      postsDirHint: "One word or hyphenated (blog), or empty for posts right under the parent page. The help icon above has the examples.",
      postsTagGroup: "Group posts by their first tag",
      postsTagGroupHint: "Adds the tag after the directory: /blog/news/your-post, with a list of each tag's posts at /blog/news.",
      ...
      postsCountHint: "Newest first. 0 shows every post.",
      postsFilter: "Let visitors filter by tag",
      postsFilterKind: "Filter style",
      postsFilterKinds: { pills: "Pills", select: "Select list" },
      postsFilterKindHint: "Pills show every tag at a glance; a select list keeps a long tag set compact.",
    ...
    media: {
      ...
      detail: {
        ...
        primary: "Primary",
        makePrimary: "Make this the primary tag",
        dragTag: "Drag to reorder",
        noTags: "No tags yet.",
        alt: "Alt text",
