...
| A **parallax / scroll effect doesn't move** (nowhere, or only in the phone/tablet frame) | `background-attachment: fixed` or a **`window` scroll listener / `useScroll`**: the device frames scroll inside their own `overflowY:auto` screen, so window scroll never fires and fixed attachment is disabled inside a scroll container (a `transform` ancestor disables it everywhere); a hand-rolled `animation-timeline: view()` on an image inside an **`overflow-hidden`** section makes the section the scroll container and freezes it (`<Parallax>` converts Tailwind `overflow-hidden` ancestors to `clip` itself; an inline `overflow: hidden` style is not caught) | Grep the section for `background-attachment`, `fixed`, `addEventListener("scroll"`, `useScroll`; walk the `<Parallax>` ancestors for `overflow-hidden` | Use **`<Parallax>`** ([Parallax.tsx](src/app/components/Parallax.tsx), CSS scroll-driven, see `/design` §4 rule 8) and delete the fixed attachment / listener; change any inline `overflow: hidden` ancestor to `overflow-clip`. |
| A **brand font renders as a plain system face** (in the design preview, the site, or one but not the other) | The family named by the `--ta-font-*` token has **no @font-face / @import in the stylesheet that surface loads**: the design surface loads the base `src/styles/fonts.css` plus the active variation's `fonts.css` + `tokens.css`; the site loads the pinned variation's `fonts.css` (self-hosted at build). A family only imported elsewhere, or spelled differently than the token, never arrives | In the page, `[...document.fonts].filter(f => f.family === 'Name')` is empty (note `document.fonts.check()` returns TRUE when no face exists, so don't trust it); grep the variation's `fonts.css` for the family | Add the family's Google `@import` (or `@font-face`) to the **variation's** `styles/fonts.css` (first lines, before any rule) and match the token's spelling; never hand-copy self-hosted faces into it. |
