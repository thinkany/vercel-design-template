...
| A **parallax / scroll effect doesn't move** (nowhere, or only in the phone/tablet frame) | `background-attachment: fixed` or a **`window` scroll listener / `useScroll`**: the device frames scroll inside their own `overflowY:auto` screen, so window scroll never fires and fixed attachment is disabled inside a scroll container (a `transform` ancestor disables it everywhere); an **`overflow-hidden` wrapper** around a `<Parallax>` makes itself the scroll container and freezes its view timeline | Grep the section for `background-attachment`, `fixed`, `addEventListener("scroll"`, `useScroll`; walk the `<Parallax>` ancestors for `overflow-hidden` | Use **`<Parallax>`** ([Parallax.tsx](src/app/components/Parallax.tsx), CSS scroll-driven, see `/design` §4 rule 8) and delete the fixed attachment / listener; change the wrapper to `overflow-clip` (or drop it). |
