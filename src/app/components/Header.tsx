...
export function Header({ onNavigate }: { onNavigate: (page: string) => void }) {
  // Menu state is shared with the MobileMenu drawer via context (provided by
  // DesignSurface) so the trigger and the panel can't drift apart.
  ...
  return (
    <header data-block="header" data-block-name="Header" className="relative w-full border-b border-black/10 bg-ta-cream">
      <div className="flex items-center justify-between px-6 py-4 @lg:px-10">
        ...
        {/* Mobile trigger — placed on MENU_SIDE (order-first pulls it to the left
            edge when the menu lives left) so it sits on the same side the drawer
            slides from. The drawer itself is the MobileMenu block, rendered by
            DesignSurface as an in-frame overlay. */}
        <button
