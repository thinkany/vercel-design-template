...
export async function selfHost({ repoRoot, stylesDir, log = () => {} }) {
  ...
  // Current when the @import URLs AND the roles match: the import often already lists a
  // family the designer later switches a role to, so the URLs alone would call a manifest
  // that never hosted that family "current" and the site would fall back on it.
  if (prev && JSON.stringify(prev.urls) === JSON.stringify(urls) && JSON.stringify(prev.roles || null) === JSON.stringify(roles) && fs.existsSync(path.join(outDir, "fonts.css")) && (prev.faces || []).every((f) => fs.existsSync(path.join(repoRoot, "public", f.file)))) return prev; // current
  ...
  // Files WE hosted for an earlier design that nothing references any more. Only those:
  // public/fonts also holds fonts the designer uploaded (the app's font:install writes
  // there), which are theirs to keep.
  const keep = new Set(faces.map((f) => path.basename(f.file)));
  for (const f of prev?.faces || []) { const b = path.basename(f.file); if (!keep.has(b)) { try { fs.unlinkSync(path.join(outDir, b)); } catch {} } }
  ...
  const manifest = { urls, roles, faces, preload, generatedAt: new Date().toISOString() };
